            ...
            <?php foreach ($products as $product): ?>
                ...
                <div class="bg-white border border-neutral-200 overflow-hidden flex flex-col justify-between group">
                    <div class="relative aspect-square bg-[#f5f5f5] overflow-hidden cursor-pointer" onclick="window.location='/index.php?page=product&id=<?= $product['id'] ?>'">
                        ...
                        <button onclick="event.stopPropagation(); toggleWishlist(<?= $product['id'] ?>)" class="absolute top-2 right-2 p-1.5 bg-white/90 rounded-full text-neutral-600 hover:text-red-500 transition-colors cursor-pointer">
                    ...
                    <div class="p-3 sm:p-4 flex-1 flex flex-col justify-between space-y-3">
                        <div>
                            <h3 onclick="window.location='/index.php?page=product&id=<?= $product['id'] ?>" class="text-xs font-medium text-neutral-900 line-clamp-2 hover:text-black cursor-pointer leading-snug mb-2">
                            ...
                            <?php if (!empty($colors)): ?>
                                <div class="flex items-center gap-1.5 mb-2">
                                    <?php foreach ($colors as $c): ?>
                                        <span class="w-3 h-3 rounded-full border border-neutral-300 block" style="background-color: <?= escape($c['hex'] ?? '#ccc') ?>;" title="<?= escape($c['name'] ?? '') ?>"></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                        ...
                        <div class="grid grid-cols-2 gap-1.5 pt-2 border-t border-neutral-100">
                            <button onclick="buyNow(<?= $product['id'] ?>)" class="w-full bg-black text-white text-[10px] sm:text-[11px] font-bold uppercase py-2 px-1 hover:bg-neutral-800 transition-colors text-center tracking-tight cursor-pointer">
                                BUY NOW
                            </button>
                            <button onclick="quickView(<?= $product['id'] ?>)" class="w-full bg-white text-black border border-black text-[10px] sm:text-[11px] font-bold uppercase py-2 px-1 hover:bg-neutral-100 transition-colors text-center tracking-tight cursor-pointer">
                                QUICK VIEW
                            </button>
                        </div>
                    </div>
